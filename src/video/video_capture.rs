...
impl WindowsCaptureHandler for Capture {
    ...
    fn on_frame_arrived(
        ...
        println!(
            "Frame len: {:?}",
            frame.buffer().unwrap().as_raw_buffer().len()
        );

        //OPENH264

        // let mut yuv = YUVBuffer::new(frame_width as usize, frame_height as usize);
        // yuv.read_rgb(frame.buffer().unwrap().as_raw_buffer());
